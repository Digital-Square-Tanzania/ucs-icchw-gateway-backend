...
class GatewayController {
  ...
  static async verifyMessageFromFfars(req, res, next) {
    try {
      ...
      const result = await GatewayService.verifyMessageFromFfars(message.body, message.header, signature);
  ...
  static async verifyMessageFromUcs(req, res, next) {
    try {
      ...
      const result = await GatewayService.verifyMessageFromUcs({ message: message }, signature);
      return BaseResponse.success(res, "Signature verification result", { isVerified: result });
    } catch (error) {
      next(error);
    }
